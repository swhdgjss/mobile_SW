...
class PlayRTCStatsReportHandler(activity: PlayRTCActivity) : PlayRTCStatsReportObserver {
    ...
    init {
        this.activity = activity //영남이가 고침(JTK)
    }
    ...
    override fun onStatsReport(report: PlayRTCStatsReport) {
        ...
        val text = String.format("Local\n ICE:%s\n Frame:%sx%sx%s\n 코덱:%s,%s\n Bandwidth[%sps]\n RTT[%s]\n RttRating[%d/%.4f]\n VFLost[%d/%.4f]\n AFLost[%d/%.4f]\n\nRemote\n ICE:%s\n Frame:%sx%sx%s\n 코덱:%s,%s\n Bandwidth[%sps]\n VFLost[%d/%.4f]\n AFLost[%d/%.4f]\n",
                ...
                report.remoteAudioCodec, //영남이가 고침(JTK)
                Formatter.formatFileSize(activity!!.applicationContext, report.availableReceiveBandwidth.toLong()) + "",
                remoteVideoFl.level,
                remoteVideoFl.value,
                remoteAudioFl.level,
                remoteAudioFl.value)
        ...
        activity!!.printRtcStatReport(text)  //영남이가 고침(JTK)
    }
