...
class PlayRTCHandler(activity: PlayRTCActivity) : PlayRTCObserver() {
    ...
    fun createPlayRTC(runType: Int, channelRing: String, videoCodec: String, audioCodec: String) {

        // PlayRTC 서비스 설정 객체를 생성하여 반환
        val config = createPlayRTCConfig(runType, channelRing, videoCodec, audioCodec)
    ...
    */
     ...
     */
    private fun createPlayRTCConfig(runType: Int, channelRing: String, videoCodec: String, audioCodec: String): PlayRTCConfig {
        /* PlayRTC 서비스 설정 */
         ...
         *  - H264, Open H.264
         */
        var videPreferCodec: VideoCodec = VideoCodec.VP8
        if (videoCodec == "vp9") {
         ...
         *  - OPUS
         */
        var audioPreferCodec: AudioCodec = AudioCodec.OPUS
        if (audioCodec == "isac") {
        ...
        if (runType == 1) {
            ...
            */
            ...
            config.video.isEnable = false  /* 영상 전송 안함 */
            config.audio.isEnable = false   /* 음성 전송 안함 */
